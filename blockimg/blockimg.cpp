...
static bool discard_blocks(int fd, off_t offset, uint64_t size) {
    ...
    if (!S_ISBLK(sb.st_mode)) {
        printf("not a block device; skipping BLKDISCARD\n");
        return true;
    }
...
static int PerformBlockImageUpdate(const char* name, State* state, int argc, char * argv[],
    ...
    std::string dirname = STASH_DIRECTORY_BASE;
    struct stat sb;
    int res = stat(dirname.c_str(), &sb);

    if (res == -1 && errno != ENOENT) {
        ErrorAbort(state, kStashCreationFailure, "cache dir \"%s\" failed: %s\n",
                   dirname.c_str(), strerror(errno));
        return -1;
    } else if (res != 0) {
    ...
    length = ftell (rm);
    fseek (rm, 0, SEEK_SET);
    params.patch_start = (uint8_t *)malloc ((length+1)*sizeof(uint8_t));

    if (params.patch_start)
    {
    ...
    if (params.canwrite) {
        params.nti.entry = new_data_fn_data;
        ...
        int error = pthread_create(&params.thread, &attr, get_new_data, &params.nti);
        if (error != 0) {
    ...
    if (rmdir(STASH_DIRECTORY_BASE) == -1) {
