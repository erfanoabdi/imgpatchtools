...
static bool discard_blocks(int fd, off_t offset, uint64_t size) {
    ...
    if (status == -1) {
        printf("BLKDISCARD ioctl failed: %s\n", strerror(errno));
        return false;
    }
    return true;
}
...
static bool check_lseek(int fd, off_t offset, int whence) {
    off_t rc = TEMP_FAILURE_RETRY(lseek(fd, offset, whence));
    if (rc == -1) {
...
static ssize_t RangeSinkWrite(const uint8_t* data, ssize_t size, void* token) {
    ...
    if (rss->p_remain == 0) {
        printf("range sink write overrun");
        return 0;
    }
    ...
    while (size > 0) {
        ...
        if (rss->p_remain == 0) {
            ...
            if (rss->p_block < rss->tgt.count) {
                ...
                off_t offset = static_cast<off_t>(rss->tgt.pos[rss->p_block*2]) * BLOCKSIZE;
...
static int ReadBlocks(const RangeSet& src, std::vector<uint8_t>& buffer, int fd) {
    ...
    for (size_t i = 0; i < src.count; ++i) {
        if (!check_lseek(fd, (off_t) src.pos[i * 2] * BLOCKSIZE, SEEK_SET)) {
            return -1;
        }
...
static int WriteBlocks(const RangeSet& tgt, const std::vector<uint8_t>& buffer, int fd) {
    ...
    for (size_t i = 0; i < tgt.count; ++i) {
        off_t offset = static_cast<off_t>(tgt.pos[i * 2]) * BLOCKSIZE;
...
static int VerifyBlocks(const std::string& expected, const std::vector<uint8_t>& buffer,
    ...
    if (hexdigest != expected) {
        if (printerror) {
            printf("failed to verify blocks (expected %s, read %s)\n",
...
static int LoadStash(CommandParameters& params, const std::string& base, const std::string& id,
    ...
    if (!params.canwrite) {
        if (stash_map.find(id) != stash_map.end()) {
            ...
            if (ReadBlocks(src, buffer, params.fd) == -1) {
                printf("failed to read source blocks in stash map.\n");
                return -1;
            }
            if (VerifyBlocks(id, buffer, src.size, true) != 0) {
                printf("failed to verify loaded source blocks in stash map.\n");
                return -1;
            }
            return 0;
        }
    ...
    if ((sb.st_size % BLOCKSIZE) != 0) {
        printf("%s size %" PRId64 " not multiple of block size %d",
...
static int WriteStash(const std::string& base, const std::string& id, int blocks,
    ...
    /*if (checkspace && CacheSizeCheck(blocks * BLOCKSIZE) != 0) {
        printf("not enough space to write stash\n");
        return -1;
    }*/
    ...
    if (exists) {
        ...
        if (res == 0) {
            ...
            // are unlikely)
            printf(" skipping %d existing blocks in %s\n", blocks, cn.c_str());
            *exists = true;
            return 0;
        }
...
static int SaveStash(CommandParameters& params, const std::string& base,
    ...
    // <stash_id> <src_range>
    if (params.cpos + 1 >= params.tokens.size()) {
        printf("missing id and/or src range fields in stash command\n");
        return -1;
    }
    ...
    if (usehash && VerifyBlocks(id, buffer, blocks, true) != 0) {
        ...
        printf("failed to load source blocks for stash %s\n", id.c_str());
        return 0;
    }
    ...
    printf("stashing %zu blocks to %s\n", blocks, id.c_str());
...
static int LoadSrcTgtVersion2(CommandParameters& params, RangeSet& tgt, size_t& src_blocks,
    ...
    if (!android::base::ParseUint(token.c_str(), &src_blocks)) {
        printf("invalid src_block_count \"%s\"\n", token.c_str());
        return -1;
    }
...
static int LoadSrcTgtVersion3(CommandParameters& params, RangeSet& tgt, size_t& src_blocks,
    ...
    if (VerifyBlocks(srchash, params.buffer, src_blocks, true) == 0) {
        ...
        if (overlap && params.canwrite) {
            printf("stashing %zu overlapping blocks to %s\n", src_blocks,
                    srchash.c_str());

            bool stash_exists = false;
            if (WriteStash(params.stashbase, srchash, src_blocks, params.buffer, true,
                           &stash_exists) != 0) {
                printf("failed to stash overlapping source blocks\n");
                return -1;
            }
    ...
    printf("partition has unexpected contents\n");
...
static int PerformCommandMove(CommandParameters& params) {
    ...
    if (status == -1) {
        printf("failed to read blocks for move\n");
        return -1;
    }
    ...
    if (status == 0) {
        ...
        printf("warning: commands executed out of order [%s]\n", params.cmdname);
    }

    if (params.canwrite) {
        if (status == 0) {
            printf("  moving %zu blocks\n", blocks);

            if (WriteBlocks(tgt, params.buffer, params.fd) == -1) {
                return -1;
            }
        } else {
            printf("skipping %zu already moved blocks\n", blocks);
        }
...
static int PerformCommandFree(CommandParameters& params) {
    // <stash_id>
    if (params.cpos >= params.tokens.size()) {
        printf("missing stash id in free command\n");
        return -1;
    }
...
static int PerformCommandZero(CommandParameters& params) {
    ...
    if (params.cpos >= params.tokens.size()) {
        printf("missing target blocks for zero\n");
        return -1;
    }
...
static int PerformCommandNew(CommandParameters& params) {
    ...
    if (params.cpos >= params.tokens.size()) {
        printf("missing target blocks for new\n");
        return -1;
    }
    ...
    if (params.canwrite) {
        printf(" writing %zu blocks of new data\n", tgt.size);
        ...
        off_t offset = static_cast<off_t>(tgt.pos[0]) * BLOCKSIZE;
...
static int PerformCommandDiff(CommandParameters& params) {
    ...
    // <offset> <length>
    if (params.cpos + 1 >= params.tokens.size()) {
        printf("missing patch offset or length for %s\n", params.cmdname);
        return -1;
    }

    size_t offset;
    if (!android::base::ParseUint(params.tokens[params.cpos++].c_str(), &offset)) {
        printf("invalid patch offset\n");
        return -1;
    }
    ...
    if (status == -1) {
        printf("failed to read blocks for diff\n");
        return -1;
    }
    ...
    if (params.canwrite) {
        if (status == 0) {
            printf("patching %zu blocks to %zu\n", blocks, tgt.size);
            ...
            if (params.cmdname[0] == 'i') {      // imgdiff
                if (ApplyImagePatch(params.buffer.data(), blocks * BLOCKSIZE, &patch_value,
                    ...
                    printf("Failed to apply image patch.\n");
                    return -1;
                }
            } else {
            ...
            if (rss.p_block != tgt.count || rss.p_remain != 0) {
                printf("range sink underrun?\n");
            }
        } else {
            printf("skipping %zu blocks already patched to %zu [%s]\n",
...
static int PerformCommandErase(CommandParameters& params) {
    ...
    if (fstat(params.fd, &sb) == -1) {
        printf("failed to fstat device to erase: %s\n", strerror(errno));
        return -1;
    }

    /*if (!S_ISBLK(sb.st_mode)) {
        printf("not a block device; skipping erase\n");
        return -1;
    }*/

    if (params.cpos >= params.tokens.size()) {
        printf("missing target blocks for erase\n");
        return -1;
    }
...
static char * PerformBlockImageUpdate(const char* name, State* state, int argc, char * argv[],
    ...
    if (argc < 5){
        printf("shit\n");
        ErrorAbort(state, kArgsParsingFailure, "shit");
        return StringValue(strdup(""))->data;
    ...
    char* blockdev_filename_data = argv[1];
    char* transfer_list_value_data = argv[2];
    char* new_data_fn_data = argv[3];
    char* patch_data_fn_data = argv[4];

    ssize_t blockdev_filename_size = sizeof(argv[1]);
    ssize_t transfer_list_value_size = sizeof(argv[2]);
    ssize_t new_data_fn_size = sizeof(argv[3]);
    ssize_t patch_data_fn_size = sizeof(argv[4]);
    ...
    if (patch_entry == nullptr) {
        printf("%s(): no file \"%s\" in package", name, patch_data_fn->data);
        return StringValue(strdup(""));
    }
    ...
    if (new_entry == nullptr) {
        printf("%s(): no file \"%s\" in package", name, new_data_fn->data);
        return StringValue(strdup(""));
    }*/
    ...
    if (params.fd == -1) {
        printf("open \"%s\" failed: %s\n", blockdev_filename_data, strerror(errno));
        return StringValue(strdup(""))->data;
    }
    ...
    // processing.
    std::ifstream transfer_list(transfer_list_value_data);
    //const std::string transfer_list(transfer_list_value_data, transfer_list_value_size);
    std::vector<std::string> lines;
    for (std::string line; std::getline( transfer_list, line ); /**/ )
    ...
    if (!android::base::ParseInt(lines[0].c_str(), &params.version, 1, 4)) {
        printf("unexpected transfer list version [%s]\n", lines[0].c_str());
        return StringValue(strdup(""))->data;
    }
    ...
    if (total_blocks == 0) {
        return StringValue(strdup("t"))->data;
    }
    ...
    if (params.version >= 2) {
        ...
        printf("maximum stash entries %s\n", lines[2].c_str());
        ...
        int res = CreateStash(state, stash_max_blocks, blockdev_filename_data, params.stashbase);
        if (res == -1) {
            return StringValue(strdup(""))->data;
        }
    ...
    if (params.canwrite) {
        ...
        printf("wrote %zu blocks; expected %d\n", params.written, total_blocks);
        printf("stashed %zu blocks\n", params.stashed);
        printf("max alloc needed was %zu\n", params.buffer.size());

        const char* partition = strrchr(blockdev_filename_data, '/');
        ...
        printf("verified partition contents; update may be resumed\n");
    }
...
pbiudone:
    ...
    return StringValue(rc == 0 ? strdup("t") : strdup(""))->data;
}
...
char * BlockImageVerifyFn(const char* name, State* state, int argc, char * argv[]) {
...
char * BlockImageUpdateFn(const char* name, State* state, int argc, char* argv[]) {
    const Command commands[] = {
        { "bsdiff",     PerformCommandDiff  },
...
/*Value* RangeSha1Fn(const char* name, State* state, int , char* argv[]) {
    ...
    for (size_t i = 0; i < rs.count; ++i) {
        if (!check_lseek(fd, (off_t)rs.pos[i*2] * BLOCKSIZE, SEEK_SET)) {
...
Value* CheckFirstBlockFn(const char* name, State* state, int argc, char* argv[]) {
    Value* arg_filename;
    ...
    RangeSet blk0 {1 , 1, std::vector<size_t> {0, 1}};
    ...
    if (mount_count > 0) {
        printf("Device was remounted R/W %d times\n", mount_count);
        printf("Last remount happened on %s", ctime(&mount_time));
    }
...
/*Value* BlockImageRecoverFn(const char* name, State* state, int argc, char* argv[]) {
    Value* arg_filename;
    Value* arg_ranges;
    ...
    printf("%s image corrupted, attempting to recover...\n", filename->data);
    ...
    for (size_t i = 0; i < rs.count; ++i) {
        for (size_t j = rs.pos[i * 2]; j < rs.pos[i * 2 + 1]; ++j) {
            ...
            if (fh.pread(buffer, BLOCKSIZE, (off_t)j * BLOCKSIZE) != BLOCKSIZE) {
    ...
    printf("...%s image recovered successfully.\n", filename->data);
    return StringValue(strdup("t"));
}*/

int main(int argc, char * argv[]) {
    if (argc < 5){
        printf("usage: ./blockimg <system.img> <system.transfer.list> <system.new.dat> <system.patch.dat> [update?]\nargs:\n\t- block device (or file) to modify in-place\n\t- transfer list (blob)\n\t- new data stream (filename within package.zip)\n\t- patch stream (filename within package.zip, must be uncompressed)\n");
        return 0;
    }

    State* state;
    char * i;
    if(strncmp(argv[5],"1",1))
        i=BlockImageVerifyFn("BlockImageVerifyFn", state, argc, argv);
    else
        i=BlockImageUpdateFn("BlockImageUpdateFn", state, argc, argv);
